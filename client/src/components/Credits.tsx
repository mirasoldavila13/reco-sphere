 ...
 * Credits Component
 *
 * This component serves as a dedicated page to provide proper attribution to the TMDb API,
 * in compliance with their usage terms. It features a minimalist design with consistent
 * layout styling provided by the shared `Navbar` and `Footer` components.
 *
 * Features:
 * - **Attribution Message**:
 *   - Clearly displays a required message about TMDb API usage.
 * - **TMDb Logo**:
 *   - Uses an inline SVG to ensure compliance with TMDb's branding requirements.
 * - **Layout**:
 *   - Integrates with global `Navbar` and `Footer` components for consistent app-wide navigation.
 *   - Maintains a full-height responsive layout with flex utilities.
 *
 * Design Considerations:
 * - Utilizes `min-h-screen` to ensure the content spans the full viewport height.
 * - Centers the content vertically and horizontally for a clean, focused display.
 * - Adopts Tailwind CSS classes to match the application's overall design language.
 */
